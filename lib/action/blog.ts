...
export async function readBlog(){
    ...
    .select("*").eq("is_published",true).order("created_at", {ascending: true})
}
...
export async function readBlogContentById(blogId:string){
    const supabase = await createSupabaseServerClient();
    return supabase.from("blog")
    .select("*, blog_content(*)").eq("id",blogId).single();
}
...
export async function updateBlogDetailById(blogId:string,data:BlogFormSchemaType){
    ...
    .update(blog).eq("id", blogId);

    if(resultBlog.error){
        return JSON.stringify(resultBlog)
    } else{
        const result = await supabase.from("blog_content")
        .update({content:data.content}).eq("blog_id", blogId);
